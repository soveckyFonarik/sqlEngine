...
parse = (tokens) => {
    ...
    const expect = (token) => (peek().value == token && next()) || error(`Expect keyword ${token} insted got ${peek().value}`);
    const expectType = (t) => (peek().type == t && next()) || error(`Expect type ${t} insted got ${peek().value}`);
    const getColumnName = () => {
    ...
    const getCondition = () => {
        return { leftSide: getColumnName(), sign: expectType('cmp').value, rightSide: getField() };
    }
    ...
    if (expect('FROM')) {
        ...
        while (match('JOIN')) {
            next();
            joins.push({ table: getName(), condition: expect('ON') && getCondition() });
        }
    }
    if (match('WHERE')) {
        next();
...
//select table_name1, table_name2 from table_name
engine = (db, q) => {
    ...
    if (q.joins.length > 0) {
        q.joins.forEach(t => { result = leftJoin(result, normalize(t.table), t.condition) });
    }
...
var testengine = new SQLEngine(movieDatabase);
// var actual = testengine.execute('SELECT movie.name FROM movie');
// console.log(actual)

// actual = testengine.execute('SELECT movie.name FROM movie WHERE movie.directorID = 1');
// console.log(actual)
actual = testengine.execute('SELECT movie.name, director.name FROM movie JOIN director ON director.id = movie.directorID');
console.log(actual)